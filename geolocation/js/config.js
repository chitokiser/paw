...
/* ===== GP Pause / Speed Filter ===== */
export let pausedBySpeed=false, slowStreak=0, fastStreak=0, lastTs=null;
let pausedUntil = 0;
export function isGPActive(){ return !pausedBySpeed && Date.now() >= pausedUntil; }
...
/* ===== Tap mini-game overlay ===== */
export function ensureTapOverlay() {
  ...
  const title = document.createElement('h3'); title.textContent='Quick Hit!';
  ...
  Object.assign(hitBtn.style,{padding:'12px 18px',borderRadius:'14px',border:'0',cursor:'pointer',background:'#2563eb',color:'#fff',fontWeight:'800',fontSize:'16px',width:'100%'});
  const cancel= document.createElement('button'); cancel.id='tapCancel'; cancel.textContent='Cancel';
  Object.assign(cancel.style,{marginTop:'10px',background:'transparent',color:'#93a3b8',border:'0',cursor:'pointer',fontSize:'13px'});
  card.append(title,desc,status,hitBtn,cancel); ov.append(card); document.body.appendChild(ov); return ov;
}
export function tapChallenge(mid) {
  ...
  function finish(ok){ done=true; overlay.style.display='none'; clearInterval(timer); hitBtn.onclick=null; cancel.onclick=null; resolveFn?.(ok); }
