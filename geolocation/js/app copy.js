...
onSnapshot(collection(db, 'monsters'), (qs) => {
  ...
  qs.docChanges().forEach(ch => {
    ...
    // ✅ (1) 부활 시점이 지났는데 아직 죽음 상태면 "즉시 복구" 트랜잭션
    const dueRevive = (!alive || d.dead === true) && respawnAt > 0 && respawnAt <= now;
    if (dueRevive) {
      (async () => {
        try {
          await runTransaction(db, async (tx) => {
            const ref = doc(db, 'monsters', id);
            const snap = await tx.get(ref);
            if (!snap.exists()) return;
            const cur = snap.data() || {};
            const curAlive = (cur.alive !== false) && (cur.dead !== true);
            const curRespawnAt = Number(cur.respawnAt || 0);

            if ((!curAlive || cur.dead === true) && curRespawnAt <= Date.now()) {
              tx.update(ref, {
                alive: true,
                dead: false,
                respawnAt: 0,
          ...
          // 로컬 자동공격 차단 해제(부활 허용)
          try { monstersGuard?.killedLocal?.delete(String(id)); } catch {}
        } catch (e) {
          console.warn('revive-on-read failed:', e);
        }
      })();
    }
    ...
    if (shouldShow) {
      if (!monstersReg.has(id)) {
        ...
        if (imgChanged || rec.sizePx !== newSize) {
          ...
          rec.sizePx = newSize;
          rec.bound = false; // 아이콘 교체 → HP바/클릭 재부착 필요
        }
        rec.data = d;
