  ...
  try{
    ...
    for (const oldRef of candidates){
      ...
      if (oldSnap.exists()){
        ...
        await setDoc(newRef, { ...data, migratedFrom: oldRef.id, migratedAt: serverTimestamp() }, { merge:true });
        return true;
      }
    }
  }catch(e){ console.warn('[identity] migrateInventoryIfNeeded fail', e); }
  return false;
}
