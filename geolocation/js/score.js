...
export const Score = (() => {
  ...
  let _stats = { uid:null, level:1, hp:1000, exp:0, attack:1, defense:10, distanceM:0, maxHp:1000, cp:0 };
  ...
  const _subscribeUser = () => {
    ...
    _unsub = onSnapshot(ref, async (ss)=>{
      if (!ss.exists()){
        await setDoc(ref, {
          uid, level:1, hp:1000, exp:0, attack:1, defense:10,
      ...
      // CP: cp를 1순위로 사용(원하신 정책). cp가 없고 chainPoint>0일 때만 1회 마이그레이션.
      const cpRaw  = p.cp;
      const legacy = Number(p.chainPoint ?? 0);
      const chosen = (cpRaw == null || Number(cpRaw) <= 0) && legacy > 0 ? legacy : Number(cpRaw ?? 0);
      _stats.cp = Math.max(0, Number(chosen || 0));

      try {
        if ((cpRaw == null || Number(cpRaw) === 0) && legacy > 0) {
          await _save({ cp: _stats.cp }); // cp로 정규화
        }
      } catch (e) { console.warn('[Score] cp migrate save fail', e); }
  ...
  return {
    ...
    async setHP(next){
      ...
      _syncHUD(); await _save({ hp:_stats.hp });
    },
    async deductHP(amount=1){
      ...
      if (_stats.hp<=0){
        ...
        try{
          if (this.getCP() !== 0) await this.setCP(0);
          _toast?.('사망: CP가 0으로 리셋되었습니다.');
        }catch(e){ console.warn('[Score] reset CP on death failed', e); }

        try{ playDeath(); }catch{} try{ _playFail?.(); }catch{}
        _toast?.('기절했습니다. (HP 0)');
    ...
    async addExp(amount=0){
      ...
      if (newExp >= need){
        ...
        // 정책: maxHp는 DB 값 유지(필요 시 별도 로직으로 증가)
        _stats.hp = _maxHP();
        newExp = 0;
        leveled = true;
        try{ playReward(); }catch{}
      }
      _stats.exp = newExp; _syncHUD();
    ...
    async setCP(next){
      ...
      _syncHUD();
      await _save({ cp:_stats.cp }); // cp만 사용(요청 정책)
    },
    async addCP(delta=0){
